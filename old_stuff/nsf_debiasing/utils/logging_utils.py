...
class Logger(object):
    def __init__(self, fpath=None, console=sys.stdout, mode='w'):
    ...
    def write(self,*args,**kwargs):
        if self.file is not None:
            builtins.print(*args,**kwargs,file=self.file)
            self.file.flush()
        if self.console is not None:
            builtins.print(*args,**kwargs,file=self.console)
...
def write_dict_to_tb(writer, dict_, prefix, step):
    if prefix[-1] != "/":
        prefix += "/"
