...
from utils import load, save_model, save_pred, get_pred_prefix, get_model_prefix, collate_list, detach_and_clone, InfiniteDataIterator, get_train_loader
...
def train(algorithm, datasets, general_logger, config, epoch_offset, best_val_metric, unlabeled_dataset=None, train_grouper=None):
    ...
    for epoch in range(epoch_offset, config.n_epochs):
        ...
        if config.add_start > -1 and epoch in config.add_milestones:
            if (epoch == config.add_start) and (config.subsample_ref == 'same_across_class'):
                ...
                dataset = WILDSSubset(dataset, np.concatenate(subsample_indices), transform=None)
    ...
    for split in additional_splits:
        if config.save_best:
            prefix = get_model_prefix(datasets[split], config)
            epoch, best_val_metric = load(algorithm, prefix + 'best_model.pth', device=config.device)
...
def save_model_if_needed(algorithm, dataset, epoch, config, is_best, best_val_metric):
    ...
    if config.save_last:
        save_model(algorithm, epoch, best_val_metric, prefix + 'last_model.pth')
    if config.save_best and is_best:
        save_model(algorithm, epoch, best_val_metric, prefix + 'best_model.pth')
