...
def initialize_bert_transform(config):
    ...
    def transform(text):
        try:
            ...
            tokens = {"input_ids": torch.zeros(1, config.max_token_length, dtype=torch.long), "attention_mask": torch.zeros(1, config.max_token_length, dtype=torch.long), "token_type_ids": torch.zeros(1, config.max_token_length, dtype=torch.long)}
