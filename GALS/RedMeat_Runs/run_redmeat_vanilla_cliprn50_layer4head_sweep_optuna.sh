#!/bin/bash -l
# Vanilla RedMeat sweep with CLIP RN50 backbone, tuning layer4 + classifier head.
# No guidance loss (pure vanilla training objective).
#
# Defaults:
# - 50 Optuna trials (TPE)
# - 150 epochs
# - post rerun of best hyperparameters on 5 seeds
...
# CLIP RN50 layer4+head vanilla setup.
...
OUT_CSV=${OUT_CSV:-$LOG_DIR/redmeat_vanilla_cliprn50_layer4head_sweep_${SLURM_JOB_ID}.csv}
POST_OUT_CSV=${POST_OUT_CSV:-$LOG_DIR/redmeat_vanilla_cliprn50_layer4head_best5_${SLURM_JOB_ID}.csv}
