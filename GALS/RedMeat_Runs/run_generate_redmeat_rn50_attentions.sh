#!/bin/bash -l
# Generate CLIP RN50 Grad-CAM attentions for RedMeat (food-101-redmeat).
# Debug-partition, chunked, resumable.
...
#SBATCH --mem=16G
#SBATCH --output=/home/ryreu/guided_cnn/logsRedMeat/redmeat_rn50_attention_gen_%j.out
#SBATCH --error=/home/ryreu/guided_cnn/logsRedMeat/redmeat_rn50_attention_gen_%j.err
...
echo "[GEN] Generating CLIP RN50 attentions for RedMeat..."
for ((start=0; start<TOTAL_IMAGES; start+=CHUNK_SIZE)); do
  ...
  srun --unbuffered python -u extract_attention.py \
    --config RedMeat_Runs/configs/redmeat_attention_rn50.yaml \
    DATA.ROOT="$DATA_ROOT" \
