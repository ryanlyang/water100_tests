...
#SBATCH --mem=16G
#SBATCH --output=/home/ryreu/guided_cnn/logsRedMeat/guided_redmeat_openclip_laion_5seeds_%j.out
#SBATCH --error=/home/ryreu/guided_cnn/logsRedMeat/guided_redmeat_openclip_laion_5seeds_%j.err
...
GT_PATH=${GT_PATH:-/home/ryreu/guided_cnn/Food101/LearningToLook/code/WeCLIPPlus/results_redmeat_openclip_laion_dinovit/val/prediction_cmap}

ATTENTION_EPOCH=${ATTENTION_EPOCH:-2}
KL_LAMBDA=${KL_LAMBDA:-11.44}
KL_INCR=${KL_INCR:-0.0}
BASE_LR=${BASE_LR:-2.40e-3}
CLASSIFIER_LR=${CLASSIFIER_LR:-2.33e-4}
LR2_MULT=${LR2_MULT:-1.567}

SEEDS=${SEEDS:-"0 1 2 3 4"}
CHECKPOINT_DIR=${CHECKPOINT_DIR:-$LOG_DIR/guided_redmeat_openclip_laion_ckpts_${SLURM_JOB_ID}}
SUMMARY_CSV=${SUMMARY_CSV:-$LOG_DIR/guided_redmeat_openclip_laion_5seeds_${SLURM_JOB_ID}.csv}
...
if [[ ! -f "$DATA_ROOT/all_images.csv" ]]; then
  echo "[ERROR] Missing all_images.csv under DATA_ROOT: $DATA_ROOT" >&2
  exit 1
fi
if [[ ! -d "$GT_PATH" ]]; then
...
for seed in $SEEDS; do
  run_log="$LOG_DIR/guided_redmeat_openclip_laion_seed${seed}_${SLURM_JOB_ID}.log"
  echo "=== seed=$seed ==="

  python - "$DATA_ROOT" "$GT_PATH" "$seed" "$SUMMARY_CSV" "$run_log" "$CHECKPOINT_DIR" \
    "$ATTENTION_EPOCH" "$KL_LAMBDA" "$KL_INCR" "$BASE_LR" "$CLASSIFIER_LR" "$LR2_MULT" <<'PY' 2>&1 | tee "$run_log"
import csv
import sys
from types import SimpleNamespace

import RedMeat_Runs.run_guided_redmeat as rgr
...
rgr.SEED = seed
rgr.base_lr = base_lr
rgr.classifier_lr = classifier_lr
rgr.lr2_mult = lr2_mult
rgr.checkpoint_dir = checkpoint_dir
...
best_val, test_acc, per_group, worst_group, ckpt = rgr.run_single(args, attn_epoch, kl_lambda, kl_incr)
...
echo "[DONE] Guided RedMeat rerun complete."
