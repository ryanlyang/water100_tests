#!/bin/bash
# Submit the six RedMeat systematic teacher-corruption conditions.
...
WORKER="${WORKER:-$SCRIPT_DIR/run_redmeat_r4rr_systematic_corruption_condition.sh}"
...
try:
    ...
    valid = (
        ...
        and summary.get("dataset") == "redmeat"
...
job_name_for_condition() {
  case "$1" in
    random_20pct) printf 'r4c_meat_rand' ;;
    class_baby_back_ribs) printf 'r4c_meat_ribs' ;;
    class_filet_mignon) printf 'r4c_meat_filet' ;;
    class_pork_chop) printf 'r4c_meat_pork' ;;
    class_prime_rib) printf 'r4c_meat_prime' ;;
    class_steak) printf 'r4c_meat_steak' ;;
  esac
}
...
echo "  cd $SCRIPT_DIR"
echo "  python summarize_redmeat_r4rr_systematic_corruption.py --run-root $RUN_ROOT"
