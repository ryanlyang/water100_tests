#!/usr/bin/env python3
"""Optuna/random sweep for RedMeat NSF.

The objective is validation balanced class accuracy, reported with the same
``balanced_group``/``worst_group`` field names used by the other RedMeat
runners. RedMeat does not expose explicit spurious-group labels in this setup,
so these are class-balanced and worst-class metrics.
"""
...
from RedMeat_Runs import run_nsf_redmeat as nsf  # noqa: E402
...
def _fmt_class_acc(class_acc: np.ndarray) -> str:
    return ",".join(f"{float(x):.4f}" for x in class_acc.tolist())
...
def _run_trial(trial_id: int, args, rng: np.random.Generator, sampler_name: str) -> Dict:
    ...
    return {
        ...
        "test_group_acc": _fmt_class_acc(result["test_group_acc"]),
...
def main() -> None:
    p = argparse.ArgumentParser(description="Optuna/random sweep for RedMeat NSF.")
    p.add_argument("data_root", help="RedMeat root containing all_images.csv")
    ...
    p.add_argument("--checkpoint-dir", default="NSF_RedMeat_Checkpoints")
    ...
    p.add_argument("--output-csv", default="nsf_redmeat_sweep.csv")
    p.add_argument("--post-seeds", type=int, default=5)
    p.add_argument("--post-seed-start", type=int, default=0)
    p.add_argument("--post-output-csv", default="nsf_redmeat_best5.csv")
