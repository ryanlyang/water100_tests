...
if [[ "$FORCE_EXTRACT" -eq 1 || ! -d "$ATT_PATH" ]]; then
  ...
  ATT_JOB_ID=$(sbatch --parsable --export=ALL run_waterbirds100_extract_attention_vit.sh)
  echo "[SUBMIT] Attention job id: $ATT_JOB_ID"
  echo "[SUBMIT] Stage 2: GALS ViT sweep (afterok:$ATT_JOB_ID)"
  SWEEP_JOB_ID=$(sbatch --parsable --dependency=afterok:"$ATT_JOB_ID" --export=ALL run_waterbirds100_gals_sweep_vit.sh)
  ...
  SWEEP_JOB_ID=$(sbatch --parsable --export=ALL run_waterbirds100_gals_sweep_vit.sh)
  echo "[SUBMIT] Sweep job id: $SWEEP_JOB_ID"
fi
