#!/bin/bash -l
# Retrain/load the Waterbirds-95 ElRep seed-4 checkpoint and run Pointing Game.
...
#SBATCH --mem=32G
#SBATCH --output=/home/ryreu/guided_cnn/logsWaterbird/elrep95_seed4_pointing_game_%j.out
#SBATCH --error=/home/ryreu/guided_cnn/logsWaterbird/elrep95_seed4_pointing_game_%j.err
...
MASK_ROOT="${MASK_ROOT:-/home/ryreu/guided_cnn/waterbirds/LearningToLook/code/WeCLIPPlus/results/val/prediction_cmap}"
...
# Fixed Waterbirds-95 ElRep best hyperparameters from elrep95_best5_21360853.csv.
ELREP_SEED="${ELREP_SEED:-4}"
BASE_LR="${BASE_LR:-0.019630442695504173}"
CLASSIFIER_LR="${CLASSIFIER_LR:-7.267191608584487e-05}"
THETA1="${THETA1:-3.332582224436613e-05}"
THETA2="${THETA2:-1.8101732764960995e-05}"
...
CKPT_DIR="${CKPT_DIR:-$REPO_ROOT/ElRep_Checkpoints/wb95_seed4_pointing_game}"
OUTPUT_DIR="${OUTPUT_DIR:-$LOG_DIR/elrep95_seed4_pointing_game_${SLURM_JOB_ID:-local_$(date +%Y%m%d_%H%M%S)}}"
...
PG_ARGS=(
  ...
  --seed 0
  --methods elrep
  --wb95-data-path "$DATA_PATH"
  --wb95-mask-root "$MASK_ROOT"
  --elrep95-ckpt "$ELREP_CKPT"
