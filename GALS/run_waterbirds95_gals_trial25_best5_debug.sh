#!/bin/bash -l
# Re-run Waterbirds95 GALS (RN50-attention config) with fixed Trial-25
# hyperparameters for 5 seeds and report mean/std summary.
...
#SBATCH --mem=32G
#SBATCH --output=/home/ryreu/guided_cnn/logsWaterbird/waterbirds95_gals_trial25_best5_%j.out
#SBATCH --error=/home/ryreu/guided_cnn/logsWaterbird/waterbirds95_gals_trial25_best5_%j.err
...
# Fixed Trial-25 hyperparameters (Waterbirds95 GALS RN50 sweep)
BASE_LR=${BASE_LR:-0.022040465005914615}
CLS_LR=${CLS_LR:-0.0003017377081829659}
GRAD_WEIGHT=${GRAD_WEIGHT:-24143.93955532796}
...
OUT_CSV="$LOG_DIR/gals95_trial25_best5_${SLURM_JOB_ID}.csv"
TRIAL_LOGS="$LOG_DIR/gals95_trial25_best5_logs_${SLURM_JOB_ID}"
AGG_CSV="$LOG_DIR/gals95_trial25_best5_summary_${SLURM_JOB_ID}.csv"
SUMMARY_TXT="$LOG_DIR/gals95_trial25_best5_summary_${SLURM_JOB_ID}.txt"
mkdir -p "$TRIAL_LOGS"
...
for seed in "${SEEDS[@]}"; do
  ...
  RUN_PREFIX="gals95_trial25_seed${seed}_${SLURM_JOB_ID}"
  echo "[RUN] seed=$seed run_prefix=$RUN_PREFIX"
  srun --unbuffered python -u run_gals_sweep.py \
    --method gals \
    --config configs/waterbirds_95_gals.yaml \
    --data-root "$DATA_ROOT" \
...
with open(summary_txt, "w") as f:
    f.write("WB95 GALS RN50 Trial-25 fixed hyperparams (5 seeds)\n")
...
echo "[DONE] Waterbirds95 GALS RN50 Trial-25 fixed reruns complete."
