#!/bin/bash -l
# Run one Waterbirds-95 R4RR teacher-corruption condition over seeds 0-4.
...
CONDITION="${CONDITION:?Submit with a Waterbirds-95 group or matched-random CONDITION}"
case "$CONDITION" in
  group_land_on_land|random_matched_land_on_land|\
  group_land_on_water|random_matched_land_on_water|\
  group_water_on_land|random_matched_water_on_land|\
  group_water_on_water|random_matched_water_on_water) ;;
...
TEACHER_MAP_PATH="${TEACHER_MAP_PATH:-/home/ryreu/guided_cnn/Food101/LearningToLook/code/WeCLIPPlus/results_waterbirds95_openclip_laion_dinovit/val/prediction_cmap}"
HPARAMS_CONFIG="${HPARAMS_CONFIG:-$GALS_ROOT/RightForTheRightRegions/configs/r4rr_optimized_hparams.yaml}"
PYTHON_RUNNER="${PYTHON_RUNNER:-$GALS_ROOT/RightForTheRightRegions/repro_runs/r4rr/ablations/r4rr_waterbirds95_systematic_teacher_corruption.py}"
...
echo "[RUN] dataset=waterbirds95 condition=$CONDITION seeds=$SEEDS_CSV"
echo "[RUN] corruption_seed=$CORRUPTION_SEED"
...
echo "[LOCKED] base_lr=4.82e-5 classifier_lr=2.93e-3 attention_epoch=109 kl_lambda=295.30 lr2_mult=0.409 kl_increment=0"
which python

[[ -f "$DATA_ROOT/metadata.csv" ]] || {
  echo "[ERROR] Missing Waterbirds-95 metadata: $DATA_ROOT/metadata.csv" >&2
  exit 2
}
[[ -d "$TEACHER_MAP_PATH" ]] || {
