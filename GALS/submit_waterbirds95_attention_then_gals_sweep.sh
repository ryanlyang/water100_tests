...
if [[ "$FORCE_EXTRACT" -eq 1 || ! -d "$ATT_PATH" ]]; then
  ...
  ATT_JOB_ID=$(sbatch --parsable --export=ALL run_waterbirds95_extract_attention.sh)
  echo "[SUBMIT] Attention job id: $ATT_JOB_ID"
  echo "[SUBMIT] Stage 2: GALS sweep (afterok:$ATT_JOB_ID)"
  SWEEP_JOB_ID=$(sbatch --parsable --dependency=afterok:"$ATT_JOB_ID" --export=ALL run_waterbirds95_gals_sweep.sh)
  ...
  SWEEP_JOB_ID=$(sbatch --parsable --export=ALL run_waterbirds95_gals_sweep.sh)
  echo "[SUBMIT] Sweep job id: $SWEEP_JOB_ID"
fi
