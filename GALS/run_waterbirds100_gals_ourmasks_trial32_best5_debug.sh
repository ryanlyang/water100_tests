#!/bin/bash -l
# Re-run Waterbirds100 GALS+ourmasks with fixed Trial-32 hyperparameters for 5 seeds.
...
#SBATCH --mem=32G
#SBATCH --output=/home/ryreu/guided_cnn/logsWaterbird/waterbirds100_gals_ourmasks_trial32_best5_%j.out
#SBATCH --error=/home/ryreu/guided_cnn/logsWaterbird/waterbirds100_gals_ourmasks_trial32_best5_%j.err
...
MASK_DIR=${MASK_DIR:-/home/ryreu/guided_cnn/waterbirds/L100/LearningToLook/code/WeCLIPPlus/results/val/prediction_cmap}
MASK_DIR="${MASK_DIR%\"}"
MASK_DIR="${MASK_DIR#\"}"
MASK_DIR="${MASK_DIR%\'}"
MASK_DIR="${MASK_DIR#\'}"

# Fixed Trial-32 hyperparameters (Waterbirds100 GALS+ourmasks sweep)
BASE_LR=${BASE_LR:-0.027832168464276615}
CLS_LR=${CLS_LR:-1.0338288372941498e-05}
GRAD_WEIGHT=${GRAD_WEIGHT:-60190.65400934708}
...
OUT_CSV="$LOG_DIR/gals100_ourmasks_trial32_best5_${SLURM_JOB_ID}.csv"
TRIAL_LOGS="$LOG_DIR/gals100_ourmasks_trial32_best5_logs_${SLURM_JOB_ID}"
mkdir -p "$TRIAL_LOGS"
rm -f "$OUT_CSV"
...
for seed in "${SEEDS[@]}"; do
  ...
  RUN_PREFIX="gals100_ourmasks_trial32_seed${seed}_${SLURM_JOB_ID}"
  echo "[RUN] seed=$seed run_prefix=$RUN_PREFIX"
  srun --unbuffered python -u run_gals_sweep.py \
    --method gals \
    --config configs/waterbirds_100_gals_ourmasks.yaml \
    --data-root "$DATA_ROOT" \
