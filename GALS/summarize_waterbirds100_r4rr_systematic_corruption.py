#!/usr/bin/env python3
"""Combine Waterbirds-100 R4RR corruption-condition summaries."""
...
CLASS_ORDER = ("Landbird", "Waterbird")
CLASS_KEYS = tuple(name.lower() for name in CLASS_ORDER)
GROUP_ORDER = (
    "Land_on_Land",
    "Land_on_Water",
    "Water_on_Land",
...
PAIRS = tuple(
    (f"class_{key}", f"random_matched_{key}", name)
    for key, name in zip(CLASS_KEYS, CLASS_ORDER)
)
...
def main():
    ...
    for condition in CONDITIONS:
        ...
        if summary.get("dataset") != "waterbirds100" or summary.get("condition") != condition:
            raise RuntimeError(f"Unexpected summary identity: {summary_path}")
        if summary.get("class_order") != list(CLASS_ORDER):
            raise RuntimeError(f"Unexpected class order: {summary_path}")
        ...
        row = {
            ...
            "target_class": summary["target_class_name"],
    ...
    output_csv = run_root / "waterbirds100_systematic_corruption_all_conditions.csv"
    atomic_write_csv(output_csv, list(rows[0]), rows)

    all_seed_rows = []
    for condition in CONDITIONS:
        all_seed_rows.extend(per_seed_by_condition.get(condition, []))
    all_seed_output = run_root / "waterbirds100_systematic_corruption_all_seeds.csv"
    if all_seed_rows:
        atomic_write_csv(all_seed_output, list(all_seed_rows[0]), all_seed_rows)

    pair_rows = []
    for systematic, random_control, target_class in PAIRS:
        ...
        if set(systematic_rows) != set(range(5)) or set(random_rows) != set(range(5)):
            raise RuntimeError(f"Incomplete paired seed rows for {target_class}")
        for seed in range(5):
            ...
            if systematic_row["corrupted_example_count"] != random_row["corrupted_example_count"]:
                raise RuntimeError(f"Corruption counts are not matched for {target_class}")
            pair_rows.append(
                {
    ...
    pair_output = run_root / "waterbirds100_systematic_vs_random_paired_seeds.csv"
    if pair_rows:
    ...
    for row in rows:
        print(
            f"{row['condition']:<28s} {int(row['n_seeds']):>1d} "
