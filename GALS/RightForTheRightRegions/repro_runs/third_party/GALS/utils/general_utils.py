...
def torch_load_compat(path, map_location=None, weights_only=False):
    """
    Back/forward compatible torch.load wrapper.
    PyTorch >=2.6 defaults weights_only=True; many project artifacts require
    weights_only=False. Older versions do not accept the weights_only kwarg.
    """
    kwargs = {}
...
def check_module_state_dict(original_state_dict, force_remove_module=False):
    ...
    if visible.strip():
        num_gpus = len([d for d in visible.split(',') if d.strip()])
    else:
...
def init_wandb(config):
    # Hard-disable wandb for all local/repro runs in this repository.
