...
class Base():
    def __init__(self, config, dataloaders):
        ...
        # Select CUDA by actual runtime availability, not env-var presence.
    ...
    def move_model_to_device(self):
        ...
        if self.device == 'cuda':
            ...
            if visible.strip():
                num_visible = len([d for d in visible.split(',') if d.strip()])
            else:
                num_visible = torch.cuda.device_count()
        if self.device == 'cuda' and num_visible > 1:
    ...
    def load_checkpoint(self, checkpoint_file):
        map_location = self.device if self.device in ('cpu', 'cuda') else 'cpu'
        checkpoint = gu.torch_load_compat(
            checkpoint_file,
