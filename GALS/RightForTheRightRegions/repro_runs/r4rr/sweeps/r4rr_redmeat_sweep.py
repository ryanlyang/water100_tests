...
# Import local train runner from repro_runs/r4rr/train.
REPRO_ROOT = Path(__file__).resolve().parents[2]
REDMEAT_TRAIN_ROOT = REPRO_ROOT / "r4rr" / "train"
if str(REDMEAT_TRAIN_ROOT) not in sys.path:
    sys.path.insert(0, str(REDMEAT_TRAIN_ROOT))
import r4rr_redmeat as rgm  # noqa: E402
...
def summarize_post_by_teacher_map(rows, summary_csv=None):
    metrics = ("best_balanced_val_acc", "test_acc", "per_group", "worst_group")
    grouped = {}
    for row in rows:
        teacher_map = row.get("teacher_map_path")
        if teacher_map:
            grouped.setdefault(teacher_map, []).append(row)

    out_rows = []
    for teacher_map_path, teacher_map_rows in grouped.items():
        for metric in metrics:
            vals = [row.get(metric) for row in teacher_map_rows]
    ...
    if out_rows:
        print("[POST] Per-teacher-map-path summary over post seeds:")
        for row in out_rows:
            print(
                f"  teacher_map_path={row['teacher_map_path']} | {row['metric']}: "
                f"{row['mean']:.4f} +/- {row['std']:.4f} (n={row['n']})"
            )

    if summary_csv:
        header = ["teacher_map_path", "metric", "mean", "std", "n"]
...
def _run_single_with_params(args, seed, teacher_map_path, attn_epoch, kl_lambda, kl_incr, base_lr, classifier_lr, lr2_mult):
    rgm.SEED = int(seed)
...
def main():
    ...
    p.add_argument("teacher_map_path", help="Primary teacher-map root for sweep + primary post-seed reruns")
    ...
    p.add_argument(
        "--post-summary-csv",
        default=None,
        help="Optional CSV to store per-teacher-map-path mean/std summary over post-seed reruns.",
    )
    p.add_argument(
        "--alt-teacher-map-path",
        action="append",
        default=[],
        help="Additional teacher-map roots to rerun best hyperparameters on after primary reruns. Can be provided multiple times.",
    )
    ...
    if not os.path.isdir(args.teacher_map_path):
        raise FileNotFoundError(f"Missing primary teacher_map_path: {args.teacher_map_path}")
    ...
    if args.post_seeds > 0:
        ...
        eval_roots = [args.teacher_map_path] + [p for p in args.alt_teacher_map_path if p and os.path.isdir(p)]

        for pth in args.alt_teacher_map_path:
            if pth and not os.path.isdir(pth):
                print(f"[POST] Skipping missing alt teacher-map path: {pth}")
        ...
        for root_idx, teacher_map_root in enumerate(eval_roots):
            phase = "best_primary" if root_idx == 0 else f"best_alt_{root_idx}"
            print(f"[POST] Rerunning best hyperparameters on teacher-map root ({phase}): {teacher_map_root}")
            for s in seeds:
                t0 = time.time()
                best_balanced_val, test_acc, per_group, worst_group, ckpt = _run_single_with_params(
                    args=args,
        ...
        summarize_post_by_teacher_map(post_rows, summary_csv=summary_csv)
