    ...
    for epoch in range(num_epochs_val):
        ...
        for phase in ["train", "val"]:
            ...
            for batch in dataloaders[phase]:
                # Metadata split returns (img, label, path); folder fallback also returns (img, label, path)
                if len(batch) != 3:
                    raise RuntimeError(f"Unexpected batch format for CE baseline: len={len(batch)}")
...
def run_single_trial(args, trial_number, base_lr, classifier_lr, momentum):
    ...
    if os.path.exists(metadata_path):
        ...
        # Fallback for folder-only layout.
        full_train = base.ImageFolderWithPaths(
    ...
    print(
        f"\n=== TRIAL {trial_number}: baseline CE-only | "
    ...
    if group_acc is not None:
        print(f"[TRIAL {trial_number}] TEST per_group={per_group:.2f}% worst_group={worst_group:.2f}%", flush=True)
    ...
    row = {
        ...
        "per_group": float(per_group) if per_group is not None else None,
        "worst_group": float(worst_group) if worst_group is not None else None,
...
def main():
    p = argparse.ArgumentParser(
        description=(
            "Optuna sweep for Waterbirds ViT ERM baseline (CE-only), using the same "
            "ViT backbone style as lgmstyle_sgd_640 but with R4RR logic disabled."
        )
    )
    p.add_argument("data_path", help="Waterbirds dataset root")
    p.add_argument("teacher_map_path", help="Unused in baseline script (kept for CLI parity)")

    p.add_argument("--n-trials", "--n_trials", dest="n_trials", type=int, default=50)
    p.add_argument("--seed", type=int, default=0)
    p.add_argument("--output-csv", "--output_csv", dest="output_csv", default="guided_waterbirds_vit_lgmstyle_sgd_640_baseline_optuna50.csv")
    ...
    print(
        "[SWEEP CONFIG] baseline CE-only (R4RR off) | "
