#!/usr/bin/env python3
"""Single-run Waterbirds MobileNetV2 + R4RR runner."""
...
def parse_args():
    p = argparse.ArgumentParser(description="R4RR Waterbirds runner with MobileNetV2 student.")
    p.add_argument("data_path", help="Waterbirds dataset root")
    p.add_argument("teacher_map_path", help="Teacher-map root folder")

    p.add_argument("--seed", type=int, default=0)
    p.add_argument("--attention-epoch", "--attention_epoch", dest="attention_epoch", type=int, default=200)
...
def main():
    args = parse_args()
    common.run_guided_waterbirds(args, int(args.attention_epoch), float(args.kl_lambda), args.kl_increment)
