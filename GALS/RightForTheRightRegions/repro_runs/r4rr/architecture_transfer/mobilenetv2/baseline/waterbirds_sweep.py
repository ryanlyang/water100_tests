#!/usr/bin/env python3
"""Optuna sweep for Waterbirds MobileNetV2 CE-only baseline."""
...
def _run_trial(trial, args):
    ...
    result = common.run_baseline_waterbirds(run_args)
    return {
...
def main():
    p = argparse.ArgumentParser(description="Optuna sweep for Waterbirds MobileNetV2 CE baseline.")
    p.add_argument("data_path")
    p.add_argument("teacher_map_path", nargs="?", default=None, help="Unused; kept for CLI parity with guided runners.")
    p.add_argument("--n-trials", "--n_trials", dest="n_trials", type=int, default=50)
    p.add_argument("--seed", type=int, default=0)
    p.add_argument("--output-csv", "--output_csv", dest="output_csv", default="waterbirds_mobilenetv2_ce_sweep.csv")
    p.add_argument("--checkpoint-dir", "--checkpoint_dir", dest="checkpoint_dir", default="MobileNetV2_CE_Checkpoints")
    ...
    print(
        "[SWEEP CONFIG] Waterbirds MobileNetV2 CE | "
    ...
    def objective(trial):
        ...
        print(
            ...
            f"worst_group={row['worst_group']:.2f} base_lr={row['base_lr']:.6g} "
