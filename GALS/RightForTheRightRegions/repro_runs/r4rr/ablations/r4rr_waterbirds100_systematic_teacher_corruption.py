#!/usr/bin/env python3
"""Run one Waterbirds-100 R4RR systematic teacher corruption condition.
...
CLASS_KEYS = tuple(name.lower() for name in CLASS_NAMES)
CLASS_TO_INDEX = {key: index for index, key in enumerate(CLASS_KEYS)}
CONDITIONS = tuple(
    condition
    for key in CLASS_KEYS
    for condition in (f"class_{key}", f"random_matched_{key}")
)
...
def _target_class_key(condition: str) -> str:
    if condition.startswith("class_"):
        key = condition[len("class_") :]
    ...
    if key not in CLASS_TO_INDEX:
        raise ValueError(f"Unknown target class key {key!r}")
    return key
...
def select_corruption_indices(
    ...
    labels: Sequence[int],
    corruption_seed: int,
    matched_class_counts: Sequence[int],
) -> np.ndarray:
    if condition not in CONDITIONS:
        raise ValueError(f"Unknown condition {condition!r}; expected one of {CONDITIONS}")
    target_key = _target_class_key(condition)
    target_index = CLASS_TO_INDEX[target_key]
    labels_array = np.asarray(labels, dtype=np.int64)
    expected_count = int(matched_class_counts[target_index])

    if condition.startswith("class_"):
        selected = np.flatnonzero(labels_array == target_index)
    else:
        rng = np.random.default_rng(int(corruption_seed))
        selected = rng.choice(len(labels_array), size=expected_count, replace=False)
...
def _class_count_dict(labels: Sequence[int]) -> Dict[str, int]:
    values = np.bincount(np.asarray(labels, dtype=np.int64), minlength=len(CLASS_NAMES))
...
def validate_dataset_contract(
    ...
    for split in ("train", "val", "test"):
        ...
        observed_classes = _class_count_dict(labels)
        expected_classes = {
            name: int(EXPECTED_CLASS_COUNTS[split][index])
            for index, name in enumerate(CLASS_NAMES)
        }
        if observed_classes != expected_classes:
            raise RuntimeError(
                f"Unexpected {split} class counts: {observed_classes}; "
...
def build_manifest(
    ...
    selected = select_corruption_indices(
        ...
        matched_class_counts=EXPECTED_CLASS_COUNTS["train"],
    )
    target_key = _target_class_key(condition)
    target_index = CLASS_TO_INDEX[target_key]
    target_name = CLASS_NAMES[target_index]
    condition_type = (
        "systematic_class" if condition.startswith("class_") else "matched_random_control"
    )
    expected_count = EXPECTED_CLASS_COUNTS["train"][target_index]
    ...
    selected_class_counts = _class_count_dict(train_labels[selected])
    selected_group_counts = _group_count_dict(train_groups[selected])
    if condition_type == "systematic_class":
        expected_selected = {
    ...
    manifest: Dict[str, object] = {
        ...
        "target_class_key": target_key,
        "target_class_name": target_name,
        "matched_class_count": int(expected_count),
        ...
        "split_class_counts": {
            split: _class_count_dict(datasets[split].labels) for split in datasets
        },
...
def load_optimized_hparams(config_path: Path) -> Dict[str, float]:
    ...
    raw = config["r4rr_optimized_hparams"]["waterbirds100"]
    hparams = {
    ...
    for key, expected_value in expected.items():
        if abs(float(hparams[key]) - float(expected_value)) > 1e-12:
            raise RuntimeError(
                f"Expected optimized Waterbirds-100 {key}={expected_value}, got {hparams[key]}"
            )
    return hparams
...
def write_condition_summaries(
    ...
    summary: Dict[str, object] = {
        ...
        "target_class_index": manifest["target_class_index"],
        "target_class_key": manifest["target_class_key"],
        "target_class_name": manifest["target_class_name"],
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Five-seed Waterbirds-100 R4RR systematic teacher corruption condition"
    )
...
def main() -> None:
    ...
    if int(args.batch_size) != FIXED_BATCH_SIZE:
        raise ValueError("The locked Waterbirds-100 protocol uses batch_size=96")
    ...
    print(
        f"[SETUP] matched_class={manifest['target_class_name']} "
