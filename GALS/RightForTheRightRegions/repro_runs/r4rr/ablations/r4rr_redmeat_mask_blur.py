...
def run_single(args, attn_epoch, kl_value, kl_increment=None):
    ...
    blur_fraction = float(getattr(args, "blur_fraction", 0.15))
    if blur_fraction > 1.0:
        if blur_fraction <= 100.0:
            blur_fraction /= 100.0
        else:
            raise ValueError(
                f"--blur-fraction must be in [0, 1] or [0, 100] as percent, got {blur_fraction}"
            )
    if blur_fraction < 0.0:
        raise ValueError(f"--blur-fraction must be non-negative, got {blur_fraction}")
    blur_seed = SEED if getattr(args, "blur_seed", None) is None else int(args.blur_seed)
    blur_kernel_size = _normalize_blur_kernel_size(getattr(args, "blur_kernel_size", 31))
    blur_sigma = float(getattr(args, "blur_sigma", 10.0))
    if blur_sigma <= 0.0:
        raise ValueError(f"--blur-sigma must be > 0, got {blur_sigma}")
