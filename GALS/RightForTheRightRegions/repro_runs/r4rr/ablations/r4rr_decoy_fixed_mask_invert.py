...
def _select_fixed_subset(n_items, fraction, seed):
...
def _invert_and_renormalize_mask(mask):
...
def train_one_seed(args, seed, full_train_guided, full_train_plain, true_test, device, loader_kwargs):
    ...
    invert_seed = int(args.invert_seed) if args.invert_seed is not None else int(args.split_seed)
    invert_train_pos = _select_fixed_subset(len(train_idx), args.invert_fraction, invert_seed)
    invert_base_idx = [int(train_idx[p]) for p in sorted(invert_train_pos)]
    full_train_guided.set_inverted_indices(invert_base_idx)
    print(
        f"[MASK_INVERT] seed={seed} train split: inverted {len(invert_base_idx)}/{len(train_idx)} "
        f"({len(invert_base_idx)/max(len(train_idx),1):.2%}) | invert_seed={invert_seed}"
    )
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Seed for selecting the fixed subset of inverted teacher maps (default: --split-seed).",
    )
    ...
    if invert_fraction > 1.0:
        if invert_fraction <= 100.0:
            ...
            raise ValueError(
                f"--invert-fraction must be in [0, 1] or [0, 100] as percent, got {args.invert_fraction}"
            )
    if invert_fraction < 0.0:
        raise ValueError(f"--invert-fraction must be non-negative, got {args.invert_fraction}")
    args.invert_fraction = invert_fraction
    ...
    print(
        ...
        f"invert_fraction={args.invert_fraction} invert_seed={args.invert_seed if args.invert_seed is not None else args.split_seed}"
    )
