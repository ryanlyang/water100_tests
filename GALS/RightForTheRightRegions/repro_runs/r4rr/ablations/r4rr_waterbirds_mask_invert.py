...
def _select_fixed_subset(n_items: int, fraction: float, seed: int):
    if n_items <= 0 or fraction <= 0.0:
        return set()
    if fraction > 1.0:
        raise ValueError(f"invert_fraction must be in [0, 1], got {fraction}")
    n_invert = int(round(n_items * fraction))
    n_invert = max(1, min(n_items, n_invert))
    rng = np.random.default_rng(seed)
    chosen = rng.choice(n_items, size=n_invert, replace=False)
    return set(int(i) for i in chosen.tolist())


def _invert_and_renormalize_mask(mask: torch.Tensor) -> torch.Tensor:
    inv = torch.clamp(1.0 - mask, min=0.0)
    total = float(inv.sum().item())
    if total <= 1e-12:
        inv = torch.full_like(inv, 1.0 / float(inv.numel()))
    else:
        inv = inv / total
    return inv
...
class GuidedImageFolder(Dataset):
    def __init__(self, image_root: str, mask_root: str, image_transform=None, mask_transform=None):
        ...
        self.inverted_indices = set()

    def set_inverted_indices(self, indices):
        self.inverted_indices = set(int(i) for i in indices)
    ...
    def __getitem__(self, idx):
        ...
        if idx in self.inverted_indices:
            mask = _invert_and_renormalize_mask(mask)
...
def run_single(args, attn_epoch, kl_value, kl_increment=None):
    ...
    invert_fraction = float(args.invert_fraction)
    if invert_fraction > 1.0:
        if invert_fraction <= 100.0:
            invert_fraction /= 100.0
        else:
            raise ValueError(
                f"--invert_fraction must be in [0, 1] or [0, 100] as percent, got {args.invert_fraction}"
            )
    if invert_fraction < 0.0:
        raise ValueError(f"--invert_fraction must be non-negative, got {args.invert_fraction}")
    invert_seed = SEED if args.invert_seed is None else int(args.invert_seed)
    ...
    if os.path.exists(metadata_path):
        ...
        if use_attention and invert_fraction > 0.0:
            inverted = _select_fixed_subset(len(train_dataset), invert_fraction, invert_seed)
            train_dataset.set_inverted_indices(inverted)
            print(
                f"[MASK_INVERT] train split: inverted {len(inverted)}/{len(train_dataset)} "
                f"({len(inverted)/max(len(train_dataset),1):.2%}) | seed={invert_seed}"
            )
        else:
            train_dataset.set_inverted_indices(set())
        ...
        if use_attention and invert_fraction > 0.0:
            train_pos = sorted(_select_fixed_subset(len(train_subset), invert_fraction, invert_seed))
            base_indices = [int(train_subset.indices[p]) for p in train_pos]
            full_train.set_inverted_indices(base_indices)
            print(
                f"[MASK_INVERT] train subset: inverted {len(base_indices)}/{len(train_subset)} "
                f"({len(base_indices)/max(len(train_subset),1):.2%}) | seed={invert_seed}"
            )
        else:
            full_train.set_inverted_indices(set())
    ...
    if invert_fraction > 0.0 and not use_attention:
        print("[MASK_INVERT] Attention is disabled for this run; inverted maps are configured but not used.")
    ...
    print(
        ...
        f"invert_fraction={invert_fraction}, invert_seed={invert_seed} ===",
        flush=True
    )
    ...
    print(f"[RUN DONE] kl={kl_value} attn={attn_epoch} lr2_mult={lr2_mult} kl_incr={kl_increment} "
          f"invert_fraction={invert_fraction} invert_seed={invert_seed} | best_balanced_val_acc={best_score:.4f} "
...
def main():
    ...
    parser.add_argument('--invert_fraction', type=float, default=0.15,
                        help='Fraction of training teacher maps to invert once before training starts (0..1). '
                             'Values in 1..100 are interpreted as percent (e.g., 15 => 15%).')
    parser.add_argument('--invert_percent', dest='invert_fraction', type=float,
                        help='Alias for --invert_fraction using percent-style values.')
    parser.add_argument('--invert_seed', type=int, default=None,
                        help='Seed for selecting the fixed subset of inverted teacher maps (default: --seed).')
