...
def _normalize_blur_kernel_size(kernel_size):
    k = int(kernel_size)
    if k < 3:
...
def _blur_and_renormalize_mask(mask, kernel_size, sigma):
...
class GuidedImageFolder(utils.Dataset):
    ...
    def set_blur_params(self, kernel_size, sigma):
...
def train_one_seed(args, seed, full_train_guided, full_train_plain, true_test, device, loader_kwargs):
    ...
    blur_seed = int(args.blur_seed) if args.blur_seed is not None else int(args.split_seed)
    blur_train_pos = _select_fixed_subset(len(train_idx), args.blur_fraction, blur_seed)
    blur_base_idx = [int(train_idx[p]) for p in sorted(blur_train_pos)]
    full_train_guided.set_blurred_indices(blur_base_idx)
    print(
        f"[MASK_BLUR] seed={seed} train split: blurred {len(blur_base_idx)}/{len(train_idx)} "
        f"({len(blur_base_idx)/max(len(train_idx),1):.2%}) | blur_seed={blur_seed} "
        f"| k={full_train_guided.blur_kernel_size} sigma={full_train_guided.blur_sigma}"
    )
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.15,
        help="Fraction of training teacher maps to blur before training (0..1). "
    ...
    parser.add_argument(
        ...
        dest="blur_fraction",
        type=float,
        help="Alias for --blur-fraction using percent-style values.",
    )
    parser.add_argument(
        "--blur-seed",
        type=int,
        default=None,
        help="Seed for selecting the fixed subset of blurred teacher maps (default: --split-seed).",
    )
    parser.add_argument(
        "--blur-kernel-size",
        type=int,
    ...
    if blur_fraction > 1.0:
        if blur_fraction <= 100.0:
            ...
            raise ValueError(
                f"--blur-fraction must be in [0, 1] or [0, 100] as percent, got {args.blur_fraction}"
            )
    if blur_fraction < 0.0:
        raise ValueError(f"--blur-fraction must be non-negative, got {args.blur_fraction}")
    args.blur_fraction = blur_fraction
    args.blur_kernel_size = _normalize_blur_kernel_size(args.blur_kernel_size)
    if float(args.blur_sigma) <= 0.0:
        raise ValueError(f"--blur-sigma must be > 0, got {args.blur_sigma}")
    args.blur_sigma = float(args.blur_sigma)
    ...
    full_train_guided.set_blur_params(args.blur_kernel_size, args.blur_sigma)
    ...
    print(
        ...
        f"blur_fraction={args.blur_fraction} blur_seed={args.blur_seed if args.blur_seed is not None else args.split_seed} "
        f"blur_kernel_size={args.blur_kernel_size} blur_sigma={args.blur_sigma}"
    )
