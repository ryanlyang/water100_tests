...
def run_single(args, attn_epoch, kl_value, kl_increment=None):
    ...
    invert_fraction = float(getattr(args, "invert_fraction", 0.15))
    if invert_fraction > 1.0:
        if invert_fraction <= 100.0:
            invert_fraction /= 100.0
        else:
            raise ValueError(
                f"--invert-fraction must be in [0, 1] or [0, 100] as percent, got {invert_fraction}"
            )
    if invert_fraction < 0.0:
        raise ValueError(f"--invert-fraction must be non-negative, got {invert_fraction}")
    invert_seed = SEED if getattr(args, "invert_seed", None) is None else int(args.invert_seed)
...
def main():
    ...
    p.add_argument(
        ...
        type=float,
        default=0.15,
        help="Fraction of training teacher maps to invert before training (0..1). "
             "Values in 1..100 are interpreted as percent.",
    )
    p.add_argument(
        "--invert-percent",
        dest="invert_fraction",
        type=float,
        help="Alias for --invert-fraction using percent-style values.",
    )
    p.add_argument(
        "--invert-seed",
        type=int,
