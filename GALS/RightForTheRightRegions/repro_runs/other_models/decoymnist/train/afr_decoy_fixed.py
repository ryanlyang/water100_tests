...
def run(args: argparse.Namespace) -> None:
    ...
    print(
        ...
        "report=test_acc/test_balanced_class_acc/test_worst_class_acc at selected epoch"
    )
    ...
    for seed in seeds:
        ...
        print(
            ...
            f"test_balanced_class_acc@val={row['test_balanced_acc_at_val_worst']:.4f} "
    ...
    test_bals = np.asarray([float(r["test_balanced_acc_at_val_worst"]) for r in rows], dtype=np.float64)
    ...
    print(f"test_balanced@val_worst:   {test_bals.mean():.4f} +/- {test_bals.std():.4f}")
