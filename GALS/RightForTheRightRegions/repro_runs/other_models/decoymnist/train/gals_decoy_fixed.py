...
def evaluate_with_class_stats(
    model: nn.Module, loader: utils.DataLoader, device: torch.device, num_classes: int = 10
...
def main() -> None:
        ...
        torch.save(
            {
                ...
                "test_balanced_class_acc": float(test_balanced_class_acc),
                "test_worst_class_acc": float(test_worst_class_acc),
                "test_class_acc": [float(x) for x in np.asarray(test_class_acc, dtype=np.float64).tolist()],
                "args": vars(args),
            },
            ckpt_path,
        )

        rows.append((seed, best_val_acc, test_acc, test_balanced_class_acc, test_worst_class_acc, best_epoch))
        print(
            f"seed={seed} best_val_acc={best_val_acc:.2f}% "
            f"best_epoch={best_epoch} test_acc={test_acc:.2f}% "
            f"test_balanced_class_acc={test_balanced_class_acc:.2f}% "
    ...
    test_bals = np.asarray([r[3] for r in rows], dtype=np.float64)
    test_worsts = np.asarray([r[4] for r in rows], dtype=np.float64)
    print("\nSummary over seeds")
