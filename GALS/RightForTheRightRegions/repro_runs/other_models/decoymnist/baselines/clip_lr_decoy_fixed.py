...
def _evaluate(
    ...
    *,
    ...
    return {
        ...
        # DecoyMNIST uses class-wise metrics; keep both legacy group names and class aliases.
        "val_balanced_class_acc": float(np.nanmean(val_group)),
        "val_worst_class_acc": float(np.nanmin(val_group)),
        "val_class_accs": _fmt_arr(val_group),
        ...
        "test_balanced_class_acc": float(np.nanmean(test_group)),
        "test_worst_class_acc": float(np.nanmin(test_group)),
        "test_class_accs": _fmt_arr(test_group),
    }
...
def main() -> None:
    ...
    for i, seed in enumerate(seeds):
        ...
        print(
            ...
            f"val_balanced_class_acc={row['val_balanced_class_acc']:.4f} "
            f"test_acc={row['test_acc']:.4f} "
            f"test_balanced_class_acc={row['test_balanced_class_acc']:.4f} "
            f"test_worst_class_acc={row['test_worst_class_acc']:.4f}"
        )
