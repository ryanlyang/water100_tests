#!/usr/bin/env python3
"""Run Waterbirds-95 GALS our-masks sweep without Slurm/sbatch."""
...
def main() -> None:
    ...
    parser.add_argument("--config", default="configs/waterbirds_95_gals_ourmasks.yaml")
    ...
    out_csv = Path(args.output_csv).expanduser().resolve() if args.output_csv else logs_dir / f"gals95_ourmasks_sweep_{job_tag}.csv"
    trial_logs = Path(args.trial_logs).expanduser().resolve() if args.trial_logs else logs_dir / f"gals95_ourmasks_sweep_logs_{job_tag}"
