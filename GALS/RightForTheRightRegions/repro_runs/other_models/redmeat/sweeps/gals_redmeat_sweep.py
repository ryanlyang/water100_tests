...
def get_override_value(overrides, key):
    key_eq = f"{key}="
    for ov in (overrides or []):
        if ov.startswith(key_eq):
            return ov[len(key_eq):]
    return None
...
def run_one_trial(
    ...
    *,
    ...
    with open(trial_log, "w") as lf:
        ...
        for line in proc.stdout:
            ...
            if parsed:
                k, v = parsed
                if (
                    k.endswith("_test_acc")
                    or k.startswith("test_acc_")
                    or k in ("test_acc", "balanced_test_acc", "balanced_val_acc", "test_worst_class_acc")
                ):
                    test_metrics[k] = v
    ...
    # For datasets without explicit groups (e.g., Food subset), fall back to class-wise test metrics.
    group_keys = [
    ...
    worst_group = None
    if group_vals:
        per_group = float(np.mean(group_vals))
        worst_group = float(np.min(group_vals))
    else:
        worst_class = test_metrics.get("test_worst_class_acc")
        balanced_test = test_metrics.get("balanced_test_acc")
        test_acc = test_metrics.get("test_acc")
        if worst_class is not None:
            worst_group = float(worst_class)
            per_group = float(balanced_test) if balanced_test is not None else float(worst_class)
        elif balanced_test is not None:
            per_group = float(balanced_test)
            worst_group = float(balanced_test)
        elif test_acc is not None:
            per_group = float(test_acc)
            worst_group = float(test_acc)
...
def main():
    ...
    override_attention_dir = get_override_value(args.overrides, "DATA.ATTENTION_DIR")
    if override_attention_dir:
        inferred_attention_dir = override_attention_dir
