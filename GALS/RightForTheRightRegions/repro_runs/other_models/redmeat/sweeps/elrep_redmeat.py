...
def _gals_repo_root() -> Path:
    return Path(__file__).resolve().parents[3] / "third_party" / "GALS"
        ...
        for phase in ["train", "val"]:
            ...
            for images, labels, _paths in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(is_train):
                    outputs, features = forward_resnet_with_features(model, images)
                ...
                running_elrep_loss += rep_loss.item() * images.size(0)
...
def run_single(args):
    ...
    if save_checkpoints:
        ...
        ckpt_name = f"elrep_redmeat_{args.model}_theta1{args.theta1:g}_theta2{args.theta2:g}_seed{args.seed}_{ts}.pth"
...
def parse_args():
    p = argparse.ArgumentParser(
        description="ElRep RedMeat trainer: ERM + nuclear/Frobenius penalties on ResNet-50 penultimate features."
    )
