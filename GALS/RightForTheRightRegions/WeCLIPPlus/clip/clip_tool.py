...
def perform_single_voc_cam(img_path, image, image_features, attn_weight_list, seg_attn, bg_text_features,
    ...
    # Try to load a VOC XML; if missing, pretend the object is the whole image
    ...
    if os.path.exists(xmlfile):
        ...
        # no ground‐truth XML: treat the full image as one object
        data = {
            'size': {'width': str(w), 'height': str(h)},
            'object': [
                {
                  'name': cls,
                  'bndbox': {'xmin': 0, 'ymin': 0, 'xmax': w, 'ymax': h}
                }
                for cls in class_names  # e.g. ['tench','chainsaw']
            ]
        }
