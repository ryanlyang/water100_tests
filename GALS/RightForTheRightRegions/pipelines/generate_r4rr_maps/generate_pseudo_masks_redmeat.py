...
def _resolve_paths(repo_root, voc_workspace_root=None, workspace_name="redmeat"):
...
def _prepare_redmeat_dataset(split_images_dir: str, class_name: str, set_dir: str, dest_dir: str) -> Dict[str, int]:
    """
    Ingest split_images/{train,val}/<class>/*.jpg with IDs <source_class>_<image_stem>.
    Training lists include train split only; evaluation lists include train+val.
    """
    os.makedirs(set_dir, exist_ok=True)
    os.makedirs(dest_dir, exist_ok=True)

    split_roots = [
        ("train", os.path.join(split_images_dir, "train")),
        ("val", os.path.join(split_images_dir, "val")),
    ]
    ...
    for split_name, split_root in split_roots:
        for src_class, image_path in _iter_class_images(split_root):
            ...
            val_basenames.append(unique_id)
            if split_name == "train":
                train_basenames.append(unique_id)

    val_basenames = sorted(val_basenames)
    ...
    if not train_basenames:
        raise RuntimeError(
            f"No train images found under: {os.path.join(split_images_dir, 'train')}"
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Uses split_images/train for training and split_images/train+val for inference outputs."
        )
    )
    parser.add_argument(
        "--repo-root",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Default: <repo-root>/pipelines/generate_r4rr_maps/voc_workspaces/redmeat"
        ),
    )
    parser.add_argument(
        "--split-images-dir",
    ...
    parser.add_argument(
        ...
        dest="setup_data",
        action="store_true",
        help="Prepare VOC-style JPEGImages/ImageSets (train list=train split only; val list=train+val).",
    )
    parser.add_argument(
        "--no-setup-data",
        dest="setup_data",
    ...
    try:
        main(
            ...
            voc_workspace_root=args.voc_workspace_root,
        ...
        print(
            f"[generate_pseudo_masks_redmeat] {_status} in "
