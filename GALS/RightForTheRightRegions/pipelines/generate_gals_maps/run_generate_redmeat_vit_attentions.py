#!/usr/bin/env python3
"""Generate RedMeat GALS attention maps with CLIP ViT-B/32 (no Slurm required)."""
...
def _resolve_config(gals_root: Path, config_arg: str, candidates: list[str]) -> Path:
    ...
    raise FileNotFoundError(
        "Could not find a RedMeat ViT attention config. Checked: " + ", ".join(candidates)
    )
...
def main() -> None:
    ...
    parser.add_argument("--save-folder", type=str, default="clip_vit_attention")
    ...
    config = _resolve_config(
        ...
        args.config,
        [
            "RedMeat_Runs/configs/redmeat_attention_vit.yaml",
            "configs/redmeat_attention_vit.yaml",
    ...
    print("[DONE] RedMeat ViT attention generation complete.")
