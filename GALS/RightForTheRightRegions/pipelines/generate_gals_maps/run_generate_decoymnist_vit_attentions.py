#!/usr/bin/env python3
"""Generate GALS-style CLIP ViT attention maps for DecoyMNIST.
...
def _ensure_gals_import_path(gals_root: Path) -> None:
    gals_root = gals_root.expanduser().resolve()
    if not gals_root.is_dir():
        raise FileNotFoundError(f"Missing GALS root: {gals_root}")
    gals_root_str = str(gals_root)
    if gals_root_str not in sys.path:
        sys.path.insert(0, gals_root_str)
...
def _run_chunked_subprocesses(
    *,
    ...
    print(
        f"[INFO] Chunked mode enabled: total={total} chunk_size={chunk_size} chunks={n_chunks}",
        flush=True,
    )
    for chunk_id, chunk_start in enumerate(range(start_idx, end_idx, chunk_size), start=1):
        ...
        print(
            f"[RUN {chunk_id}/{n_chunks}] range=[{chunk_start}, {chunk_end})",
            flush=True,
        )
        subprocess.run(cmd, check=True)
    print("[DONE] All chunks completed.", flush=True)


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate CLIP ViT attentions for DecoyMNIST"
    )
    ...
    parser.add_argument(
        "--gals-root",
        type=str,
        default=str(_default_gals_root()),
        help="Path to vendored GALS root (contains CLIP/ and utils/).",
    )
    ...
    parser.add_argument(
        ...
        default=1000,
        help=(
            "If >0, process [start-idx, end-idx) in subprocess chunks of this size "
            "to avoid long-run CUDA memory growth. Use 0 to disable."
        ),
    )
    parser.add_argument("--chunk-worker", action="store_true", help=argparse.SUPPRESS)
    ...
    _set_seed(args.seed)
    device = args.device
    _ensure_gals_import_path(Path(args.gals_root))
    ...
    try:
        ...
        raise ModuleNotFoundError(
            "Failed to import GALS modules (CLIP/utils). "
            f"Resolved --gals-root={Path(args.gals_root).expanduser().resolve()}"
        ) from exc
    ...
    output_root = (
        ...
        if args.output_root
        else (png_root / "clip_vit_attention").resolve()
    )
    ...
    if (
        ...
        and chunk_size > 0
        and (end_idx - start_idx) > chunk_size
    ):
        _run_chunked_subprocesses(
            start_idx=start_idx,
            end_idx=end_idx,
    ...
    print("[DONE] DecoyMNIST ViT attention generation complete.")
