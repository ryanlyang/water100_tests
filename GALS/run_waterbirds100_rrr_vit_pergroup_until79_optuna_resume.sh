#!/bin/bash -l
# Resume-like continuation for WB100 RRR ViT per_group sweep from prior CSV trials.
...
#SBATCH --mem=32G
#SBATCH --output=/home/ryreu/guided_cnn/logsWaterbird/wb100_rrr_vit_pergroup_until79_resume_%j.out
#SBATCH --error=/home/ryreu/guided_cnn/logsWaterbird/wb100_rrr_vit_pergroup_until79_resume_%j.err
...
STOP_THRESHOLD=${STOP_THRESHOLD:-79.0}

RESUME_CSV=${RESUME_CSV:-}
if [[ -z "$RESUME_CSV" ]]; then
  RESUME_CSV="$(ls -t "$LOG_DIR"/wb100_rrr_vit_pergroup_until79_*.csv 2>/dev/null | head -n 1 || true)"
fi
if [[ -z "$RESUME_CSV" ]]; then
...
TRIAL_LOGS=${TRIAL_LOGS:-$LOG_DIR/wb100_rrr_vit_pergroup_until79_resume_logs_${SLURM_JOB_ID}}
RUN_NAME_PREFIX=${RUN_NAME_PREFIX:-wb100_rrr_vit_pergroup_until79_resume_${SLURM_JOB_ID}}
...
srun --unbuffered python -u run_gals_sweep.py \
  --method rrr \
  --config configs/waterbirds_100_gals_vit.yaml \
  --data-root "$DATA_ROOT" \
