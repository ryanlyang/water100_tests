#!/bin/bash
# Submit one independent five-day, 50-trial job per alternative alignment loss.

set -Eeuo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RUNNER="$SCRIPT_DIR/run_waterbirds100_r4rr_alignment_sweep.sh"
LOSSES=(reverse_kl jensen_shannon squared_l2 cosine)
SUBMISSION_CSV=${SUBMISSION_CSV:-/home/ryreu/guided_cnn/logsWaterbird/wb100_r4rr_alignment_submissions_$(date +%Y%m%d_%H%M%S).csv}
