#!/bin/bash -l
# UpWeight (class-weighted) baseline sweep for Waterbirds-95.
# Sweeps: EXP.BASE.LR, EXP.CLASSIFIER.LR, EXP.WEIGHT_DECAY.
#
# Default behavior:
# - 100 Optuna trials (TPE)
# - keep best trial checkpoint only
...
#SBATCH --mem=32G
#SBATCH --output=/home/ryreu/guided_cnn/logsWaterbird/waterbirds95_upweight_sweep_%j.out
#SBATCH --error=/home/ryreu/guided_cnn/logsWaterbird/waterbirds95_upweight_sweep_%j.err
...
OUT_CSV="$LOG_DIR/upweight95_sweep_${SLURM_JOB_ID}.csv"
TRIAL_LOGS="$LOG_DIR/upweight95_sweep_logs_${SLURM_JOB_ID}"

ARGS=(--method upweight
  --config configs/waterbirds_95_upweight.yaml
  --data-root "$DATA_ROOT"
